export default {
  JSE001: "Please select %s!",
  JSE002: "%s is greater than %s!",
  JSE003: "Delete Successfully!",
  JSE004: "Please choose you want to Delete!",
  JSE005: "Please fill %s!",
  JSE007: "%s is already exists!",
  JSE009: "The maximum upload file is not more than 3 file!", 
  JSE010: "Please fill Leave Name English Character Only!",
  JSE011: "%s is greater than 100!",
  JSE012: "Please fill %s is between 0 to 1!",
  JSE013: "Please fill %s English Number Only!",
  JSE014: "%s is greater than %s!",
  JSE015: "%s is greater than 60 minutes!",
  JSE016: "Please fill %s Positive Number Only!",
  JSE017: "%s is less than %s!",
  JSE018: "Please fill %s!",
  JSE019: "Please choose %s!",
  JSE020: "Please choose %s Only!",
  JSE021: "Please choose you want to Delete!",
  JSE022: "Duplicate files are not allowed!",
  JSE023: "%s and %s should not be same!",
  JSE024: "This file type is not supported!",
  JSE025: "Please choose %s or remove %s",
  JSE026: "%s is number only!",
  JSE027: "%s is at least %s",
  JSE028: "Invalid %s!",
  JSE029: "Incorrect %s!",
  JSE030: "Please fill number from %s to %s!",
  JSE031: "%s label type needs at least one add field label!",
  JSE032: "Duplicate %s are not allowed!",
  JSE033: "%s must be less than %s characters!",
  JSE034: "%s is already taken!",
  JSE035: "You need to create %s with at least %s!",
  JSE036: "Password and Confirm Password need to be same!",
  JSE037: "Password length must be at least 8 characters!",
  JSE038: "Password length must not exceed 20 characters!",
  JSE039: "Passport Number length must not exceed 12 characters!",
  JSE040: "Password is Weak!",
  JSE041: "One Attach File Can Accept Only %s Files!",
  JSE042: "Only %s can be create!",
  JSE043: "You need to %s not to use this information to another sections!", 
  JSE044: "Password must be contained capital letter, small letter, special characters and numbers!",
  JSE045: "This label can be created only in %s!",
  JSE046: "Invalid %s in %s field!",
  JSE047: "%s Does Not Match!",
  JSE048: "Start Date must be set before End date!",
  JSE049: "From Value must be less than To Value!",
  JSE050: "From Exp Year must be less than To Exp Year!",
  JSE051: "Please upload %s!",
  JSE052: "Please select %s level!",
  JSE053: "Attach file can accept only 3 files!",
  JSE054: "Attach file can accept only 1 file!",
  JSE055: "%s length must not exceed 19 characters!",
  JSE056: "%s must be between %s and %s characters!",
  JSE057: "You can add maximum 3 fields for search condition!"
};
